...
const CodeBlock = ({ inline, className, children, ...props }) => {
  // Safely check if className exists before matching language

  const match = (r) => r.split('\n').length > 3 && r.includes('```') ? `\`\`\`\n${r}\n\`\`\`` : r;

  if (inline) {
    return (
      <code
        {...props}
        className="px-1 py-0.5 rounded bg-neutral-100 text-[0.79rem] inline"
      >
        {children}
      </code>
    );
  ...
  return (
    <div className="relative w-full my-2">
      <pre
        {...props}
        className={`text-[0.79rem] w-full overflow-x-auto dark:bg-zinc-900 p-4 border border-zinc-700 rounded-xl ${
      ...
      <div className="max-w-[92%]">
        <span
          className={`p-4 px-5 rounded-2xl mb-2 inline-block ${
            message.isYou ? "bg-blue-600 text-white" : "bg-[#2a2a2a] text-white"
          } break-words`}
          style={{ wordBreak: "break-word" }} /* Adding it inline for extra insurance */
        >
          {message.isYou ? (
            message.text
          ) : (
